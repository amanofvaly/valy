import { listProducts, listProductsOfType } from "@lib/data/products"
...
export default function PartTemplate({
  ...
  return (
    <div className="container-page py-8 lg:py-12" data-testid="product-container">
        ...
        <div className="flex flex-col gap-8">
          ...
          {!!fits.length && (
            <Suspense fallback={null}>
              <FitsList handles={fits} countryCode={countryCode} />
            </Suspense>
          )}
        </div>
      </div>
      ...
      <Suspense fallback={null}>
        <SameCategory product={product} countryCode={countryCode} />
      </Suspense>
    </div>
  )
}
...
async function FitsList({
  ...
  const products = await listProductsOfType("machine", countryCode, 20)
...
async function SameCategory({
  ...
  const {
    ...
    countryCode,
    queryParams: { category_id: [categoryId], limit: 8 },
  })
